---
title: 'Carbon Trading'
description: 'Create a platform where users globally to instantly pay a business in Kenya from anywhere in the world using mobile money and allow users share money on the app with their friends and family'
thumbnail: '/images/carbon-trading-thumbnail.png'
coverImage: '/images/services/services-details-img.png'
badge: 'Project case study'
---

&nbsp;

Shell’s Enterprise Product Trading & Booking (EPTB) platform is a SaaS-based analytics tool used by traders, analysts, and business teams to manage product trading, booking, and logistics. The analytics module provides real-time data insights to support critical decision-making. However, users struggled with complex workflows, data overload, and usability inefficiencies, making it difficult to extract meaningful insights quickly.

![Image 1](/images/carbon-trading-1.png)

### Learn about the project - Project Overview

In 2021, the regulated carbon market reached a value of $851 billion, representing a 164% increase from 2020. This growth was driven by stricter carbon regulations, increased corporate sustainability commitments, and rising carbon prices.

Meanwhile, the voluntary carbon market is projected to grow to 1.5 to 2 gigatonnes of CO2 equivalent (GtCO2e) by 2034. This suggests increasing demand for carbon credits , driven by:

- (\*) More companies aiming for net-zero emissions.
- (\*) Growing interest in sustainable investing.
- (\*) Advancements in carbon capture technologies.

  Since Shell operates in energy trading, an evolving carbon credit market could impact trading strategies, regulatory compliance, and financial decisions. The EPTB platform could potentially integrate carbon credit analytics to help traders make informed decisions regarding emission-related costs.

![Image 2](/images/carbon-trading-2.png)

### Designing the User Experience

Shell EPTB Analytics delivers a clear, intuitive experience that helps users quickly access insights, spot trends, and drill into data, enabling faster, more informed decisions across devices.

![Image 3](/images/carbon-trading-3.png)

Problems and goal settings

![Image 4](/images/carbon-trading-4.png)

To design a robust, user-friendly trading platform that simplifies carbon credit transactions, ensures compliance with global environmental regulations, and provides advanced analytics for market insights. Shell's target is to become a net-zero emissions energy business by 2050.

![Image 5](/images/carbon-trading-5.png)

### Revealing a Dynamic Carbon Credit Market 

The report also provides insights on credit issuances and retirements in metric tonnes per month. The chart below shows a comprehensive view of cumulative credits issued by month over the past 3 years. Highest issuances are in December, both for 2022 and 2023. 

![Image 6](/images/carbon-trading-6.png)

A short, fictional profile that represents a key segment of my target audience known as Use persona was created. It includes details like demographics, goals, behaviours, and pain points, helping design teams create solutions that meet real user needs.

![Image 7](/images/carbon-trading-7.png)

An empathy map was created as a simple UX tool that helps teams understand users better by capturing what they say, think, feel, and do. It’s used to visualise the user’s perspective, uncover pain points, and identify opportunities to improve the experience.

![Image 8](/images/carbon-trading-8.png)

Data analysis & ideations

![Image 9](/images/carbon-trading-9.png)

![Image 10](/images/carbon-trading-10.png)

### Building The Information Architecture

I restructured the navigation, filtering, and data presentation to enhance usability and efficiency Key UX Enhancements.

![Image 11](/images/carbon-trading-11.png)

### Design and User testing

This is the phase where concepts are turned into interactive prototypes and then evaluated with real users. The goal is to see how people actually use the product, identify usability issues, and gather feedback to refine the design before final development.

![Image 12](/images/carbon-trading-12.png)

![Image 13](/images/carbon-trading-13.png)

![Image 14](/images/carbon-trading-14.png)

To validate our design improvements, I conducted usability testing with key users.

![Image 15](/images/carbon-trading-15.png)

### Conclusion & Learning

![Image 16](/images/carbon-trading-16.png)
